package data.scripts.campaign.econ;

public class DR_industries {
    //public static final String XLU_YARDS = "xlu_battle_yards";
    
    public static final String DR_STATION1 = "comet_Keep";
    public static final String DR_STATION2 = "moonCastle";
    public static final String DR_STATION3 = "DR_starFortress";
    public static final String DR_STATION4 = "DR_quasarCitadel";
    public static final String WHITESKYE = "DR_Whiteskye";// CREATES PUBLIC STRING FOR pmc code
    public static final String DR_WENDIGO = "DR_Wendigo";// CREATES PUBLIC STRING FOR pmc code
    public static final String DR_BLACKFLEET = "blackFleet";// CREATES PUBLIC STRING FOR pmc code
    public static final String DR_GENDARMERIE = "Gendarmerie";// CREATES PUBLIC STRING FOR pmc code Gendarmerie
    public static final String PLAYER_GENDARMERIE = "WhiteskyePlayer";// CREATES PUBLIC STRING FOR pmc code Gendarmerie
    public static final String RONIN = "ronin";// CREATES PUBLIC STRING FOR pmc code ronin
	
}
